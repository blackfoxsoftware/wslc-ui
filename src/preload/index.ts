...
const api: WslcApi = {
  ...
  containerAction: (action, id, options) => invoke('containers:action', { action, id, options }),
  pruneContainers: () => invoke('containers:prune', undefined),
  runContainer: (opts) => invoke('containers:run', opts),
  execInContainer: (id, command, options) => invoke('containers:exec', { id, command, options }),
  ...
  copyToContainer: (opts) => invoke('containers:copy', opts),
  removeImage: (ref, options) => invoke('images:remove', { ref, options }),
  ...
  createVolume: (name, vhd, labels) => invoke('volumes:create', { name, vhd, labels }),
  removeVolume: (name, force) => invoke('volumes:remove', { name, force }),
  ...
  removeNetwork: (name, force) => invoke('networks:remove', { name, force }),
  pruneNetworks: () => invoke('networks:prune', undefined),
  inspectNetwork: (name) => invoke('networks:inspect', { name }),
  connectNetwork: (opts) => invoke('networks:connect', opts),
  ...
  streamLogs: (id, options) => invoke('containers:logs', { id, options }),
